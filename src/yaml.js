...
return fs.writeFileSync(file, YAML.stringify(data, null, 1));
};


class YamlProvider extends EventEmitter {
constructor(options) { 
super()
const { path, separator, useEmit, checkUpdate, deleteEmptyObjects } = options;

this.path = path || 'ervel.yml';
...
if(path === "ervel.yml" && path !== undefined) throw new Error('path must be not same as default. If you want to use default you do not have to identify.')
...
const newFileName = fileName.replace('.yml', '-backup');
...
if (path.endsWith(".yml") && path !== null) throw new Error('Do not include file extensions in your file name.');
...
const db = YAML.parse(fs.readFileSync(this.path, 'utf-8'));
...
fs.writeFileSync(`${path}.yml`, YAML.stringify(db, null, 1));
